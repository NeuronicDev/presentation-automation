...
async function executeGeneratedOfficeJsCode(codeInput) {
  ...
  for (const slideIndex of slideIndices) {
      ...
      try {
          ...
          const dynamicFunc = new Function(`
              return (async () => {
                try {
                  ...
                  // Catch errors specifically from within PowerPoint.run
          ...
          // Await the execution of the dynamic async function
          const executionResult = await dynamicFunc();

          // Log based on the result returned by the dynamic function
          if (executionResult.success) {
            ...
            console.log(`--- Code execution FAILED for Slide Index ${slideIndex}: ${executionResult.error?.message || 'Unknown'} ---`);
              overallSuccess = false; // Mark overall process as failed if any slide fails
          }
      } catch (evalError) {
          // Catch errors related to creating or initially calling the dynamic Function itself
          console.log(`--- Code evaluation/setup error for Slide Index ${slideIndex}: ${evalError.message} ---`);
          console.error(`Dynamic function creation/evaluation failed for slide ${slideIndex}:`, evalError);
          overallSuccess = false;
      }
  } // End loop
