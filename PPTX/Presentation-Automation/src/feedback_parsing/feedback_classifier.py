...
classification_prompt = PromptTemplate(
    ...
    input_variables=["slide_number", "source", "instruction_text"]
)

classification_chain = classification_prompt | gemini_flash_llm

# === Step 2: Modernized Feedback Parser ===
def parse_feedback_instruction(instruction_item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    print(f"Received instruction: {instruction_text}")
    print(f"Received slide number: {slide_num}")
    ...
    try:
        # Use pipe-style chain invocation
        ...
        try:
            ...
            logging.error(f"Failed to decode JSON. Error: {json_err}. Raw response: {raw_response_text}")
            return None

        # Validate and return parsed JSON
        if not json_match:
            logging.error(f"No valid JSON object found in response for instruction: '{instruction_text}'")
            return None

        required_keys = ["category", "slide_number", "original_instruction", "tasks"]
        if not all(k in json_match for k in required_keys):
            logging.error(f"Parsed JSON missing keys. Found: {json_match}")
            return None

        if not isinstance(json_match["tasks"], list):
            logging.error(f"'tasks' field must be a list. Parsed: {json_match}")
            return None

        logging.info(f"Successfully parsed: Slide {slide_num}, Category: {json_match['category']}, Task count: {len(json_match['tasks'])}")
        return json_match

    except Exception as e:
        logging.error(f"Error processing instruction '{instruction_text}': {e}", exc_info=True)
        return None

# === Step 3: Batch Classifier ===
def classify_feedback_instructions(feedback_list: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    logging.info(f"Finished classification. Parsed {len(categorized_tasks)} of {len(feedback_list)} feedback items.")
