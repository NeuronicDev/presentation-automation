...
//     document.getElementById("run").onclick = extractSlideText;
//   }
// });
...
//         allSlideText.push(`Slide ${i + 1}: ${slideText.join(" ")}`);
//       }

//       console.log("Extracted Slide Text:\n", allSlideText.join("\n"));

//       // Display extracted text in Task Pane UI
//       document.getElementById("item-subject").innerText = allSlideText.join("\n\n");
//     });
//   } catch (error) {
          ...
          for (let j = 0; j < shapes.items.length; j++) {
            ...
            if (shape.textFrame) {
              try {
                ...
                if (text && text.trim() !== "") {
                  slideText.push(text); // Collect non-empty text
                }
              } catch (error) {
